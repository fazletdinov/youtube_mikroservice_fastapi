...
from video.api.v1.domain.services.video import VideoService, get_service_video
...
async def read_root(video_id: int, request: Request):
    return templates.TemplateResponse("index.html", context={"request": request, "path": video_id})
...
async def video_endpoint(
    ...
    start, end = range.replace("bytes=", "").split("-")  # type: ignore
    start: int = int(start)  # type: ignore
    end: int = int(end) if end else start + CHUNK_SIZE  # type: ignore
    with open(path, "rb") as video:
        video.seek(start)  # type: ignore
        data = video.read(end - start)  # type: ignore
