...
from users.schemas.user import UpdateUserSchema
...
class UserDAL(CrudBase):
    ...
    @override
    async def create(
        self, email: str, hash_password: bytes
    ) -> Row[tuple[User]] | Exception:
        try:
    ...
    @override
    async def update(
        self, user_id: int, body_user: UpdateUserSchema
    ) -> Row[tuple[User]] | None | Exception:
        try:
            stmt = (
                update(User)
                .where(User.id == user_id)
                .values(**body_user.model_dump())
                .returning(User.id)
            )
    ...
    ) -> Sequence[Row[tuple[User, int, int]]] | None:
        try:
            query = (
    ...
    async def get_by_email_full_info(
        self, email: str
    ) -> Row[tuple[User, int, int]] | None | Exception:
        try:
