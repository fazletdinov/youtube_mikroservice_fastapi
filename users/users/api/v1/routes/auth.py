...
from users.api.v1.domain.permissions.user import CurrentUser
from users.api.v1.schemas.token import TokenResponse
from users.api.v1.schemas.user import CreateUserSchema, ResponseUserSchema, UpdateUserSchema
from users.api.v1.domain.services.user import AuthService, get_auth_service

auth_router = APIRouter(tags=["Auth"], prefix="/auth")


@auth_router.post("/", response_model=ResponseUserSchema)
async def register(user_body: CreateUserSchema, auth_service: AuthService = Depends(get_auth_service)):
...
async def deactivate_user(user: CurrentUser, auth_service: AuthService = Depends(get_auth_service)):
